<template>
	<div class="diy_details page_poverty_alleviation_commodities" id="poverty_alleviation_commodities_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_poverty_alleviation_commodities'>
							<div_poverty_alleviation_commodities :obj="obj"></div_poverty_alleviation_commodities>
...
<script>
	import div_poverty_alleviation_commodities from "@/components/diy/div_poverty_alleviation_commodities.vue";
	...
	export default {
		...
		components:{
			...
			bar_title,
			form_editor,
			div_poverty_alleviation_commodities
		},
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "poverty_alleviation_commodities_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/poverty_alleviation_commodities/set?poverty_alleviation_commodities_id=' + obj["poverty_alleviation_commodities_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "poverty_alleviation_commodities",
							source_field: "poverty_alleviation_commodities_id",
							source_id: this.obj.poverty_alleviation_commodities_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.poverty_alleviation_commodities_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "poverty_alleviation_commodities",
					source_field: "poverty_alleviation_commodities_id",
					source_id: obj["poverty_alleviation_commodities_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/poverty_alleviation_commodities/set?poverty_alleviation_commodities_id=' + _this.obj["poverty_alleviation_commodities_id"], {
								praise_len
							}, (res) => {
								if(res.result){
