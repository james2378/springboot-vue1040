					...
					<el-col :span="8">
						<div class="card chart" >
							...
							<div v-if="!list_goods_price.length">订单没有符合条件的数据</div>
												...
												<el-col v-if="user_group == '管理员' || $check_figure('/sales_information/table')" :span="8">
						<div class="card chart">
									<newBarChart v-if="bar_obj_sales_information.values.length > 0" id="bar_obj_sales_information" :vm="bar_obj_sales_information" :title="'销售信息统计'">
							</newBarChart>
							<div v-if="!bar_obj_sales_information.values.length">销售信息没有符合条件的数据</div>
								</div>
					</el-col>
					</el-row>
...
<script>
	...
	export default {
		...
		created() {
										// 执行销售信息数据获取
			this.get_list_sales_information();
								...
								(json) => {
									if (json.result) {
										list[i].name = json.result.obj.nickname;
																...
																// 获取销售信息统计柱状图
			async get_list_sales_information() {
				let name_list = [];
				let query_str = "";
								...
								let flag = false;
												let date_flag = "其他"
																						name_list.push("订单金额");
				query_str = query_str+"order_amount"+","
									this.bar_obj_sales_information.names = name_list
				...
				if (user_group!='管理员'){
								let sqlwhere = "(";
														if (user_group=="卖家"){
						sqlwhere+= "store_account = " + user_id + " or ";
					}
						...
						await this.$get(
						"~/api/sales_information/bar_group?field="+query_str+"&groupby="+group_by_value,
						data,
						(json) => {
							if (json.result) {
								let xAxis = [];
								...
								this.bar_obj_sales_information.xAxis = xAxis;
								this.bar_obj_sales_information.values = values;
							}
							if (flag){
								this.get_nickname(this.bar_obj_sales_information.xAxis,true);
							}
						});
			},
