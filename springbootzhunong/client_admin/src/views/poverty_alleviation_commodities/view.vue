		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
            ...
            <el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="店铺账号" prop="store_account">
							...
							<!--v-if="user_group === '管理员' || (form['poverty_alleviation_commodities_id'] && $check_field('set','store_account')) || (!form['poverty_alleviation_commodities_id'] && $check_field('add','store_account'))" :disabled="disabledObj['store_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','store_account')">{{form['store_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['poverty_alleviation_commodities_id'] && $check_field('set','store_account')) || (!form['poverty_alleviation_commodities_id'] && $check_field('add','store_account'))" id="store_account" v-model="form['store_account']" :disabled="disabledObj['store_account_isDisabled']">
				...
				<el-form-item label="店铺名称" prop="store_name">
							  ...
							  v-if="user_group === '管理员' || (form['poverty_alleviation_commodities_id'] && $check_field('set','store_name')) || (!form['poverty_alleviation_commodities_id'] && $check_field('add','store_name'))" :disabled="disabledObj['store_name_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				form: {
                    ...
                    cart_img_4: "",
                    cart_img_5: "",
						"poverty_alleviation_commodities_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/poverty_alleviation_commodities/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/poverty_alleviation_commodities/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/poverty_alleviation_commodities/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/poverty_alleviation_commodities/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/poverty_alleviation_commodities/view','get');
            ...
            submit_after(){
                ...
                if (this.form[this.field]){
                    ...
                    this.$get("~/api/poverty_alleviation_commodities/get_obj?",query_form,(res)=>{
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
							customize_field.push({"field_name":"店铺账号","field_value":this.form.store_account});
                ...
                if (add_flag==='set'){
                    ...
                    // 提交事件
                    this.$post("~/api/goods/set?source_table=poverty_alleviation_commodities&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
